...
export function formatAuctionStartedTweetText(auctionId: number) {
 ...
 An auction has started for NounBrazil #${auctionId}
...
export async function formatBidMessageText(id: number, bid: Bid) {
  ...
  return `NounBrazil ${id} has received a bid of Ξ${ethers.utils.formatEther(bid.amount)} from ${bidder}`;
}
...
export function formatUpdatedGovernanceProposalStatusText(proposal: Proposal) {
  return `NounsBrazil DAO proposal #${proposal.id} (${extractProposalTitle(
    proposal,
...
/**
 * Get the PNG buffer data of a NounBrazil
 * @param tokenId The ERC721 token id
 * @returns The png buffer of the NounBrazil or undefined
 */
