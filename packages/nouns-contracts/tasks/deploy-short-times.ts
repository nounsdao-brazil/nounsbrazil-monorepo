...
task('deploy-short-times', 'Deploy all NounsBrazil contracts with short gov times for testing')
    ...
    const contracts: Record<ContractName, ContractDeployment> = {
      ...
      NounsDAOProxy: {
        ...
        validateDeployment: () => {
          ...
          if (expected !== actual) {
            throw new Error(
              `Unexpected NounsBrazil DAO proxy address. Expected: ${expected}. Actual: ${actual}.`,
            );
          }
        },
      },
